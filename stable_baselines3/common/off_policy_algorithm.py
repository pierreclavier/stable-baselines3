...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def learn(
        ...
        if self.penal != {}:
            if "var_penal_train" in self.penal.keys():
                self.var_penal_train = self.penal["var_penal_train"]
                self.std_penal_train = None

            if "std_penal_train" in self.penal.keys():
                self.std_penal_train = self.penal["std_penal_train"]
                self.var_penal_train = None

        else:
            self.std_penal_train, self.std_penal_train = None, None
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (
            ...
            unscaled_action, _ = self.predict(
                self._last_obs, deterministic=False, penal=self.penal
            )
